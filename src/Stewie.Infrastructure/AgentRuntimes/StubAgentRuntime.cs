...
public class StubAgentRuntime : IAgentRuntime
{
    ...
    /// <inheritdoc/>
    public async Task<string> LaunchAsync(AgentLaunchRequest request, CancellationToken ct = default)
    {
        ...
        var envArgs = string.Join(" ",
            $"-e RABBITMQ_HOST={request.RabbitMq.HostName}",
            $"-e RABBITMQ_PORT={request.RabbitMq.Port}",
            $"-e RABBITMQ_USER={request.RabbitMq.UserName}",
            $"-e RABBITMQ_PASS={request.RabbitMq.Password}",
            $"-e RABBITMQ_VHOST={request.RabbitMq.VirtualHost}",
            $"-e AGENT_QUEUE={request.RabbitMq.AgentQueueName}",
            $"-e AGENT_ID={request.SessionId}",
            $"-e PROJECT_ID={request.ProjectId}",
            $"-e AGENT_ROLE={request.Role}");
        ...
        _logger.LogInformation(
            ...
            containerName, request.SessionId, request.Role);
    ...
    /// <inheritdoc/>
    public async Task TerminateAsync(string containerId, CancellationToken ct = default)
    {
